...
def metropolis_hastings(n_samples=5000, jump_size=1.0):
    ...
    for _ in range(n_samples):
        ...
        #print(alpha, prob_current, prob_proposal)
        # if aplha > 1
...
samples = metropolis_hastings(n_samples=1000, jump_size=2.0)

plt.figure(figsize=(10, 6))

#plot histogram of samples
plt.hist(samples, bins=50, density=True, color='skyblue', alpha=0.7, label='Sample Distribution')

#Plot the true math function for comparision
x_range = np.linspace(-6, 6, 500)
plt.plot(x_range, target_distribution(x_range)/3.5, color='red', lw=2, label='True Function')
